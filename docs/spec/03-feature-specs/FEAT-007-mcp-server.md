...
This feature implements Bookit as an MCP (Model Context Protocol) server. It wraps the existing `@bookit/core` pipeline in an MCP `bookit_transform` tool, enabling AI assistants like Claude Desktop and Cursor to run documents through the pipeline directly, without opening the Electron UI.
...
| `AC-059` | Tool configuration is driven entirely by `BOOKIT_*` environment variables, read per invocation, avoiding Electron keytar store. | `MCP-FR-002` |
...
| `AC-061` | Output PDFs are saved deterministically to `BOOKIT_OUTPUT_DIR` (or default `~/Documents/Bookit/`), appending numeric suffixes if the file exists. | `MCP-FR-004` |
| `AC-062` | If `verbose: true`, the tool response includes timing records for each pipeline stage. | `MCP-FR-005` |
| `AC-063` | Tool run metadata is appended to `bookit-token-log.jsonl` with `inputType: "mcp"`. | `MCP-FR-006` |
...
1. **MCP Server Shell (`packages/mcp-server/src/index.ts`, `server.ts`)**
   ...
   - **Behaviors:** Initializes `@modelcontextprotocol/sdk` Server, registers `bookit_transform` tool, starts stdio listener.
   - **Done when:**
     - [ ] Server responds to `initialize` handshake over stdio
     - [ ] Server lists `bookit_transform` tool with proper JSON schema

2. **Environment Config (`packages/mcp-server/src/config/env-config.ts`)**
   - **Inputs:** `process.env`
   - **Outputs:** Typed `McpConfig` object
   - **Behaviors:** Validates `BOOKIT_ANTHROPIC_KEY` (or chosen provider), resolves output directory.
   - **Done when:**
     - [ ] Returns valid config object or throws specific structured error if required variables are missing

3. **Tool Handler (`packages/mcp-server/src/tools/bookit-transform.ts`)**
   ...
   - **Done when:**
     ...
     - [ ] Pipeline token usage logs to `~/Documents/Bookit/bookit-token-log.jsonl`

### Interface Contracts

No cross-package interface changes needed. The tool handler imports `@bookit/core` `PipelineOrchestrator` directly and acts as a consumer exactly like the Electron app.

## 4. Dependencies

- `@modelcontextprotocol/sdk` (MCP implementation)
- `@bookit/core` (Shared pipeline logic)
