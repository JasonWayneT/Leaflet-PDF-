...
Story 9.1 adds a vitest `test` script to `@bookit/core` and converts the compile-only `pipeline-orchestrator.test.ts` into real runtime tests covering: happy path, fidelity retry (fail → retry → pass), 3-failure halt, and module error halt. CI is updated to run tests.
